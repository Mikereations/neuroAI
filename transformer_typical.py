...
class Head(nn.Module):
    ...
    def __init__(self, head_size):
        ...
        self.querys = nn.Linear(n_embd, head_size, bias=False)
    ...
    def forward(self, x):
        ...
        k = self.keys(x)   # (B,T,hs)
        q = self.querys(x) # (B,T,hs)
...
class GPTLanguageModel(nn.Module):
    ...
    def forward(self, idx, locs, targets=None):
        ...
        if targets is not None:
            ...
            losses = self.loss(recon_batch, data)
            loss = losses
        else:
            loss = None
    ...
    def selective_policy(self, images) :
        ...
        for i, image in enumerate(images) :
            ...
            for j in range(20) :
                ...
                while np.sum(image[int(position[0]):int(position[0]+64), int(position[1]):int(position[1]+64)]) == 0 and got_dark == True :
                    position = self.policy()
                if np.sum(image[int(position[0]):int(position[0]+64), int(position[1]):int(position[1]+64)]) == 0 and got_dark == False :
                    got_dark = True
