...
class FC_Encoder(nn.Module):
    def __init__(self, output_size):
        ...
        self.fc1 = nn.Linear(image_size * image_size, output_size)
...
class CNN_Encoder(nn.Module):
    def __init__(self, output_size, input_size=(1, image_size, image_size)):
...
class CNN_Decoder(nn.Module):
    def __init__(self, embedding_size, input_size=(1, image_size, image_size)):
        ...
        self.deconv = nn.Sequential(
            ...
            nn.ReLU(True),
            # Should be : state size. self.output_channels x 28 x 28 82
            nn.ConvTranspose2d(self.output_channels, self.output_channels, 4, 2, 1, bias=False),
            nn.Sigmoid()
            # state size. self.output_channels x 28 x 28
            # Should be : state size. self.output_channels x 56 x 56
        )
